...
ContentManager.prototype.setupRoutes = function () {
  ...
  rest.get('/content/schema', function (req, res, next) {
    var schemas = helpers.cloneObject(contentmanager._contentSchemas);
      ...
      async.eachSeries(plugins, function (plugin, outerCallback) {
            ...
            function(callback) {
              async.eachSeries(results, function (schema, processCallback) {
                ...
                */
                ...
                switch (plugin.type) {
                  case 'component':
                    var baseAttributes = _.clone(contentmanager._contentSchemas['component'].properties);
        ...
        async.each(Object.keys(schemas), function(key, callback) {
          ...
          if (!_.isEmpty(contentModelMenus[key])) {
            if(!value.properties.menuSettings) { // for installs which don't have this defined yet...
              value.properties.menuSettings = {
                type: 'object',
                properties: contentModelMenus[key]
              };
            } else if(!value.properties.menuSettings.properties) {
  ...
  rest.post('/content/:type', function (req, res, next) {
    ...
    contentmanager.create(type, val, function (error, result) {
      if (error) {
  ...
  rest.get('/content/:type', function (req, res, next) {
    ...
    contentmanager.retrieve(type, search, options, function (error, results) {
      if (error) {
        res.statusCode = error instanceof ContentTypeError ? 400 : 500;
        return res.json({success: false, message: error.message});
      }
  ...
  rest.get('/content/:type/query', function (req, res, next) {
    ...
    async.each(
      ...
      contentmanager.retrieve(type, query, options, function (error, results) {
        if (error) {
          res.statusCode = error instanceof ContentTypeError ? 400 : 500;
          return res.json({ success: false, message: error.message });
        }
  ...
  rest.get('/content/:type/:id', function (req, res, next) {
    ...
    contentmanager.retrieve(type, { _id: id }, options, function (error, results) {
      if (error) {
        res.statusCode = error instanceof ContentTypeError ? 400 : 500;
        return res.json({ success: false, message: error.message });
      }
  ...
  rest.put('/content/:type/:id', function (req, res, next) {
    ...
    contentmanager.update(type, { _id: id }, val, function (error, updatedDocument) {
      if (error) {
        res.statusCode = error instanceof ContentTypeError ? 400 : 500;
        return res.json({ success: false, message: error.message });
      }
  ...
  rest.put('/content/:type/switch/:id/:switchid', function (req, res, next) {
    ...
    contentmanager.update(type, { _id: id }, val, function (error, updatedDocument) {
      ...
      contentmanager.update(type, { _id: switchId }, switchVal, function (error, updatedDocument) {
        if (error) {
  ...
  rest.delete('/content/:type/:id', function (req, res, next) {
    ...
    contentmanager.destroy(type, { _id: id }, function (error) {
      if (error) {
        res.statusCode = error instanceof ContentTypeError ? 400 : 500;
        return res.json({success: false, message: error.message});
      }
        ...
        async.series([
          ...
          function(callback) {
            ...
            async.eachSeries(clipboard['contentObjects'], function(item, cb) {
              ...
              contentmanager.create('contentobject', item, function (error, newItem) {
                if (error) {
                  return cb(error);
                }
                ...
                copyCourseAssets(contentmanager, {
          ...
          function(callback) {
            ...
            async.eachSeries(clipboard['articles'], function(item, cb) {
              ...
              contentmanager.create('article', item, function(error, newItem) {
                if (error) {
                  return cb(error);
                }
          ...
          function(callback) {
            ...
            async.eachSeries(clipboard['blocks'], function(item, cb) {
              ...
              contentmanager.create('block', item, function (error, newItem) {
                if (error) {
                  return cb(error);
                }
          ...
          function(callback) {
            ...
            async.eachSeries(clipboard['components'], function(item, cb) {
              ...
              contentmanager.create('component', item, function(error, newItem) {
                if (error) {
                  return cb(error);
                }
